...
export default function move(gameState) {
      ...
      for (let snake of gameState.board.snakes) {
        ...
        for (let segment of snake.body) {
          if (
            ...
            segment.y === snake.body[snake.length - 1].y
          ) {
            if (
  ...
  function averageWeight() {
    ...
    for (let cell of boardarray) {
      ...
      newWeights.push(
        (cell.weight + rightweight + leftweight + upweight + downweight) / 5,
      );
    }
  ...
  function chooseMove() {
    let myHead = gameState.you.body[0];
    let rightsqaure = boardarray.find(
      (cell) => cell.x == myHead.x + 1 && cell.y == myHead.y,
    );
    let leftsqaure = boardarray.find(
      (cell) => cell.x == myHead.x - 1 && cell.y == myHead.y,
    );
    let upsqaure = boardarray.find(
      (cell) => cell.x == myHead.x && cell.y == myHead.y + 1,
    );
    let downsqaure = boardarray.find(
      (cell) => cell.x == myHead.x && cell.y == myHead.y - 1,
    );
